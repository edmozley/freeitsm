...
- **Outbound webhooks** (`send_webhook` action): the universal way to push FreeITSM events into any external system — Slack, Teams, Discord, PagerDuty, Zapier/Make, or a home-grown endpoint. Attach it to any trigger and it POSTs a JSON payload to a configured URL. One-click **presets** (Slack / Teams / Discord) wrap a templated `message` in that platform's expected shape; **Custom** mode sends a raw JSON `body` you template yourself with `{{ticket.subject}}`-style variables, so it can match any target's contract. An optional signing **secret** HMAC-SHA256-signs the payload (`X-FreeITSM-Signature`) so the receiver can verify it. Delivery is synchronous with a short timeout (a dead URL fails fast, and the failure shows in the execution log); an async delivery queue with retries is the planned next step. Because it's just a workflow action, it inherits conditions, templating and the execution audit for free — and it's the outbound half that makes FreeITSM a first-class integration hub (the inbound half being the REST API, email and messaging ingest).
