...
- **Module Access** (`system/modules/`): Control which modules each analyst can use — granted to **individual analysts and to teams**, resolved together under a configurable policy, and **enforced server-side** (GitHub #30; mirrors the company-access model)
  - **Sources combine at one choke-point** (`getAnalystAllowedModules()`): an analyst's access is their own grant plus every team they belong to. New `can_access_all_modules` flag on analysts (default on) and teams (default off), plus a `team_modules` table mirroring `analyst_modules`. The all-access toggle also appears on the Analyst and Team forms
