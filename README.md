...
- **Modules scoped by company** — beyond tickets: **Change Management**, **Problem Management**, **Asset Management** (including agent ingest, which files each machine by its API key's company), **Knowledge** (with an internal/customer/public audience axis) and **CMDB**. Each follows the same shape — list scoped, every by-id read/write gated, REST resource scoped — and is invisible at N=1.
- **CMDB configuration items belong to one company** — with **no shared CIs**, so a CI's parent, relationships and linked-object properties must all stay within that company; the rule binds all-access staff too, so two clients' estates can't be wired together. Ticket↔CI links check both ends. Classes, class properties, relationship types and icons stay **shared**, since they describe how you model your CMDB rather than whose kit it is.
...
- **Web chat channel**: An embeddable website chat widget so visitors can raise tickets from any site, alongside email and WhatsApp. Managed under **Tickets → Settings → Web chat** (its own `tickets.webchat` capability, separate from the WhatsApp credentials tab): create a widget with a greeting, accent colour and launcher label, pin its conversations to a company, and list the site origins allowed to embed it. Saving mints a **public widget key** and a copy-paste `<script>` **embed snippet** you drop onto any site. That snippet loads a self-contained widget (`api/webchat/widget.js`) — a floating launcher + chat panel rendered inside a **Shadow DOM** so the host site's CSS can't touch it. A visitor gives their name + email (the pre-chat gate), types a message, and it's ingested onto the shared ticket membrane as a **Web chat**-origin ticket in the pinned company; analysts reply from the normal reading-pane composer and replies reach the visitor by polling. **Trust model:** the widget key is *not* a secret (it ships in the site's page source), so it can only *start* a conversation, and only from an allowlisted origin (checked via `Origin`, falling back to `Referer` for same-origin embeds). Reading or posting to an existing conversation requires its **per-conversation token**, held only in that one visitor's browser — so a leaked key never exposes anyone's chat. Basic per-IP / per-conversation rate limiting on top. Web chat is a self-hosted channel — one `messaging_channels` row (`channel_type='webchat'`, `provider='freeitsm'`, no external provider or credentials, a no-op `FreeitsmProvider` for the shared reply path) — so it reuses the existing ticket/inbox/reply pipeline wholesale. Schema: `webchat_widgets` (1:1 to its channel), `webchat_conversations` (the browser-token → ticket map) and `webchat_messages` (the pre-ticket transcript); helpers in `includes/webchat/`, config + public API under `api/webchat/`. **Per-widget controls:** point a widget at a **business-hours calendar** so it presents itself as "leave a message" out of hours (the enquiry still becomes a ticket, with your offline message shown), and — with a Knowledge AI provider configured — turn on **AI answers** that reply to visitors grounded strictly in your published Knowledge articles (it never invents answers). **Scoped twice over:** a widget can only be answered from its own company's articles plus shared ones (`knowledge_articles.tenant_id`, NULL = shared with all), and only from articles whose `audience` an author has deliberately set to `public` — a website visitor is an anonymous stranger, so `internal` (the default for every article) and `customer` articles are never reachable from a chat box. See *Knowledge article visibility* below. AI runs in one of two modes: **assist** raises the ticket immediately and posts the AI's replies alongside it for the analyst; **deflect** holds off, letting the visitor and AI chat privately with *no ticket raised* until the visitor escalates. **Escalation** offers up to two buttons — *Talk to a person* (promotes the chat to a live ticket, transcript as the opening message) and *Email me back* (opens a ticket with an AI-written summary as the body and the full chat log attached as a `.txt`); analyst replies mirror back into the AI chat window. *Roadmap: sending the offline / email-me-back reply out from the company's service-desk mailbox.*
- **Portal privacy — what the requester sees of their own ticket** (`includes/portal_visibility.php`, **Tickets → Settings → Privacy**, `Cap::TICKETS_PRIVACY`): a ticket's thread is **not** always a conversation with the requester. Forwarding to a supplier, or CCing a colleague, writes that message onto the **same ticket** (`send_email.php` `type='forward'`), and the third party's reply lands there too — so correspondence *about* the requester was visible *to* the requester, occasionally with another customer's document attached. Three options: **hide them** (default), **show the message but withhold its attachments**, or **show everything** (the behaviour before this setting existed). Because "forward" is recorded nowhere — a forward and an ordinary reply are identical in `emails` — the rule is not "was this a forward" but the question that actually matters, which also catches CCs and third-party replies and works on existing history with **no migration**: *was this message TO or FROM the person who raised the ticket?* **Enforced in the download endpoint as well as the listing** — hiding a link while the URL still works is decoration, since the id is guessable and the file is the sensitive part. Every ambiguous case resolves to **visible**, deliberately: **non-email channels are exempt** (WhatsApp/web chat are addressed by phone number or visitor id, so a requester's *email* matches nothing — comparing addresses there would hide the customer's entire conversation from their own portal, and there is no "forward" on those channels anyway), as are messages with no recipients recorded. Hiding an analyst's genuine reply is a worse failure than showing a forward: the customer is left believing nobody answered. 🔑 **One case is deliberately NOT ambiguous, and getting it wrong voided the whole policy:** a requester who genuinely **has no mailbox** (a directory user — see LDAP below) is a different thing from one whose address we merely do not know. No email can ever have been addressed to them, so every email on their ticket is third-party **by definition** and the policy **applies**. The original code treated a blank address as "cannot judge → show it", which would have shown those requesters every forward and every copied-in reply in full, whatever the setting — aimed at the people least able to notice, since they cannot be emailed. `NULL` (has none) and `''` (unknown) are now distinguished in `portalEmailInvolvesRequester()`, and the callers preserve the difference rather than casting it away. Internal notes remain hidden regardless (`is_internal = 0`), and the routing columns used for the decision are stripped from the API response — the portal never lists who else was on an email.
- **Reopen on customer reply** (`includes/ticket_reply.php`, **Tickets → Settings → General**): when a requester comes back on a ticket whose status is flagged `is_closed`, the ticket returns to your first open status. One rule shared by **every** channel a customer can arrive on — the self-service portal, a reply to a notification email, WhatsApp, and web chat — because "reply to a finished ticket" means the same thing whichever door they used; two copies would drift into a ticket that reopens on Monday (email) but not on Tuesday (portal). Before this, **nothing anywhere honoured it on any channel**, despite the requester-facing email template promising *"just reply to this email and it will be reopened"*. Defaults to **on** when the setting has never been saved, so the behaviour matches what customers were already told; reopening is non-destructive and visible, whereas a reply silently landing on a closed ticket nobody looks at again is not. Status names are never hardcoded — it reads `ticket_statuses.is_closed` and reopens to the first active non-closed status (preferring one called "Open", then the install default), so a renamed workflow still lands somewhere sane. Note the shipped defaults flag only **Closed** and not **Resolved**, so out of the box a reply to a Resolved ticket appends without reopening; mark Resolved as closed under **Statuses** if you want it included. Non-fatal by design: a reopen failure can never cost you the customer's message.
...
- **User management**: The Users page (`tickets/users.php`) lists every end user with their ticket history. Analysts can **Add** a new user (email required, display name / preferred name optional, password optional &mdash; leaving it blank creates a passwordless account the user can later claim via the self-service portal's register flow, which now confirms ownership by email before setting the password), **Edit** an existing user, or **Delete** one. On multi-company installs the modal also carries a **Company** picker (hidden at N=1) deciding where that person's self-service tickets are filed &mdash; see [Self-Service Portal](#self-service-portal-self-service). Its blank option means different things in the two modes and says so: on a **new** person it reads *"Work it out from their email address"* (the payload omits the field and the server infers it), while on an **existing** one it reads *"No company"*, a deliberate choice that is sent and stored. Delete is FK-safe: refused with a clear message if the user is the requester on any tickets or has any asset assignments, so audit history can never silently break
...
- **Database Verify** (`system/db-verify/`): checks every table/column exists and auto-creates anything missing, then adds known foreign keys **and restores every missing index**. Strictly **non-destructive** — it never deletes data.
  - **Index backfill**: a grown install only ever received the indexes db_verify was explicitly told to add, so foundational **unique keys** (e.g. `uq_users_email`, `uq_tickets_number`) could be silently absent — allowing duplicate data. db_verify now carries the **complete** index list from `freeitsm.sql` (`includes/db_verify_indexes.php`, generated by `scripts/gen_db_verify_indexes.php`) and restores any that are missing, idempotently. A unique key that can't be added because **duplicate rows already exist** is *reported* on the table's card, never force-applied (unlike orphaned FK rows, a duplicate is a judgement call — resolve them and re-run) When a foreign key can't be added because orphaned child rows exist (e.g. attachments left behind when their email was deleted), the result row explains it in plain English and offers a one-click **Fix** button that — after a confirm — deletes only the provably-orphaned rows (parent gone) via a whitelisted endpoint (`api/system/fix_orphans.php`, which also removes the orphaned attachment files), then re-runs verification so the FK adds cleanly
  - **Results are a searchable card grid**, not a flat list: one card per table, each with a module colour swatch and a status dot (OK / created / updated / error), plus a **FIX** flag where orphans need clearing. A **search box** filters by table name, a **module dropdown** narrows to one area (e.g. "Tickets (37)"), and **clicking a summary card** (OK / Created / Updated / Errors) filters the grid to just that status — click **Errors** to jump straight to the tables that need attention, click again to clear. All three filters compose. The module a table belongs to is **derived from its name** in `includes/db_verify_modules.php` (prefix rules + a short override map; unmatched tables fall to "Other"), reusing the app's module colours — so a new table usually files itself and there's no per-table list to maintain. Maintenance guide: wiki **Database-Verification-Developer-Guide**
  - **Click a card for that table's live structure** — columns (types, PK/UNIQUE badges, nullability, defaults), indexes, and foreign keys with their on-delete rules — read straight from `information_schema` by a **read-only** endpoint (`api/system/db_describe.php`), fully decoupled from the schema-creation logic so it can never mis-fire a migration. It reports what the database *currently holds*; whether a constraint is *missing* is answered by re-running verification (which backfills it) — see the [Database-Integrity](Database-Integrity) wiki page
...
- **Registration & Login**: Users register with email, name, and password. **Self-registration is off by default** — an admin turns it on under **System → Security** (`self_service_registration_enabled`); while off, the register page redirects to login and the "create an account" link is hidden, so only accounts an admin creates can use the portal. **When on, registration is confirmed by email** — submitting the form doesn't set a password or sign anyone in; instead a confirmation link is sent to the address, and the password is only applied when that link is opened (`self-service/verify-email.php`). This closes an account-takeover hole: many `users` rows are auto-created *without a password* by inbound email, web chat or ticket creation, and the old "claim a passwordless account" registration would let anyone who knew such an address set a password and inherit that person's tickets. Now the confirmation email goes to the address itself, so only its real owner can complete it. The parked (hashed) password lives in `user_verification_tokens` (24-hour, single-use, SHA-256-hashed token) until confirmed; a genuinely new email is likewise only created on confirmation. Requires a configured mailbox to send the email (registration fails closed with a clear message if none is set). Separate authentication from the analyst login using `$_SESSION['ss_user_id']`. Supports MFA (TOTP) login challenge. **⚠️ Run Database Verify** (adds `user_verification_tokens`).
- **Request catalogue** (`self-service/catalogue.php`, `api/self-service/get_catalogue.php` + `get_catalogue_form.php` + `submit_catalogue_form.php`): customers pick a form, fill it in, and it is recorded as a submission for the desk to action (visible in **Forms → Submissions**). Gated on **`forms.is_portal_visible`**, a new flag **separate from `is_active`** and defaulting to **0** — `is_active` is the analyst-side on/off, and an internal "New Starter Request" is active while having no business on a customer's menu; the default means no upgrade exposes an existing form. Toggled per form from the Forms list. The catalogue lists **leaf versions only** (forms are a `parent_form_id` chain), and `cloneForm()` **carries the flag to the new version** — without that, editing a published form would silently withdraw it, a disappearance nobody would connect to pressing Save. Submission goes through `FormsService::submitForm()` so the portal inherits the same validation rather than a copy that drifts, and the service **re-checks `is_portal_visible` itself**, so a hidden form is refused even though the adapter never looked. 🔑 The submitter is passed as a separate `$portalUserId`, **not** as `ActorContext->actorId`: `users` and `analysts` are different id spaces, `submitted_by` has no FK, and every reader LEFT JOINs it to `analysts` — writing a requester's id there would silently attribute their request to whichever analyst shared the number. They go in **different columns** (`submitted_by` / `submitted_by_user_id`), exactly one is set, and `get_submissions.php` now resolves both. ⚠️ Forms are **not multi-company yet** (no `tenant_id` on any forms table), so the catalogue is the same for every company — a known limitation.
- **Help Centre** (`self-service/help-centre.php`, `api/self-service/get_knowledge_articles.php` + `get_knowledge_article.php`): the portal's knowledge base, and **the first reader of the `Audience::CUSTOMER` rung** — built, tested and documented in #869, then unread for several releases while the editor had to label it *"(not yet in use)"*. Search, browse, article view, deep-linkable (`?id=`) with working browser Back. **Both scopes are server-derived and neither is accepted from the request**: audience is hard-coded to `CUSTOMER` (the rule `includes/webchat/ai.php` already follows — whoever is asking does not get to declare how trusted they are; `CUSTOMER` includes `PUBLIC`, the ladder being cumulative), and company comes from `users.tenant_id`, looked up per request since the portal session holds only `ss_user_id`. ⚠️ Uses `knowledgeTenantFilterForCompany()`, **never** `activeTenantFilter()` — for knowledge a NULL `tenant_id` means *shared with every company*, the inverse of tickets/assets, so the ticket-shaped helper would make every shared article silently vanish. Both endpoints check `is_published` **and** `is_archived` (archiving does not unpublish — omitting that half is how deleted articles once reached anonymous web chat). Scope is applied **in** the query, so an article you may not see is indistinguishable from one that doesn't exist. Bodies are TinyMCE HTML stored verbatim (the product has no server-side sanitiser), so the portal renders them through `safeHtmlFragment()` — the shared cleaner — and fails closed to inert text if it is missing. Nav note: this section is called **Knowledge** in the portal nav, matching what it is called everywhere else in FreeITSM, and the portal's own end-user guide is simply **Help** — so there are never two items with the same name. The scope these endpoints apply now lives in `includes/knowledge/portal_reader.php` (see below) rather than being repeated in each one.
- **Ticket deflection** (`self-service/new-ticket.php`): as a requester types a subject, matching Help Centre articles appear beneath it, so the answer can be found *before* a ticket exists. Deliberately quiet — it appears only when there is something to show, never interrupts, never blocks Submit, and logs nothing about what was typed. It calls the same endpoint as the Help Centre, so it can never surface anything the Help Centre itself would not.
- **Bulk audience tool** (`api/knowledge/knowledge_bulk_audience.php`, Knowledge list): tick articles and set *who can see them* in one action. Necessary because every article defaults to `internal`, so the Help Centre starts **empty on every install** and publishing a knowledge base otherwise meant opening articles one at a time. A thin loop over `KnowledgeService::saveArticle()`, which already updates `audience` only when that key is present, **rejects** an invalid audience outright rather than normalising it down to internal, and refuses articles belonging to a company the analyst can't reach — hand-rolling an UPDATE would have bypassed all three. Selection survives search and tag filtering (it lives in JS, not the DOM), and **partial success is reported rather than swallowed**: silently skipping three of fifty is how someone believes a document is published when it isn't.
- **Company** (`users.tenant_id`): On multi-company installs a requester **belongs to a company**, and tickets they raise in the portal are filed there. Previously the portal's ticket INSERT omitted `tenant_id` entirely, so **every** portal ticket landed in triage regardless of who raised it — the one intake path that never routed. The portal has no mailbox and no sender to route on, so unlike inbound email (`resolveTicketTenantForEmail()`) the answer is simply *whose account raised it*. Set it in **Tickets → Users** (the picker is hidden at N=1, and only offers companies the analyst can reach — `save_user.php` re-checks server-side, so the dropdown is a convenience rather than the guard). A requester with **no** company falls to `NULL` = triage, exactly as an email from an unrecognised domain does; single-company installs always get Default, so portal tickets aren't shaped differently from every other intake path. New accounts are pre-filled from the email domain by `resolveTenantForNewUser()` (registration, SSO JIT, admin create, REST `POST /users`) — an SSO provider pinned to a company outranks the domain, since it vouches for whoever it signs in. **Freemail is never guessed**: two customers share `gmail.com`, so a domain match there would be worse than no answer. Deliberately **not** re-derived at ticket time, so editing a company's registered domains can never silently re-file people who already exist. **⚠️ Run Database Verify** (adds `users.tenant_id` and pre-fills existing requesters from their domain — a one-time back-fill that runs only on the run that adds the column, so a later deliberate choice is never overwritten).
- **Shared chrome and app-consistent nav** (`self-service/includes/header.php`): four destinations — Dashboard, My Tickets, Knowledge, Help — rendered from **one array**, using the same `inbox.css` `.nav-btn` primitives as every other module (including the darker shade over the brand colour in dark mode). Raising a ticket and requesting something are deliberately **not** nav items: they are actions, so they live as buttons on the dashboard. Adding a portal page is one entry here, not a fifth copy of the header.
- **Dashboard** (`index.php`): four bands — action buttons (**New ticket**, **Request**, their explanatory text demoted to a `title` tooltip so they size like a card), **status cards generated from your configured `ticket_statuses`** rather than a hardcoded four, recent tickets beside a live system status panel from Service Status, and **popular articles**. All three grids use the same `auto-fill` tracks so they line up down the page instead of each choosing its own column width.
- **Popular articles on the dashboard** (`api/self-service/get_knowledge_articles.php?sort=popular`): the six most-read articles published to customers, ordered by `knowledge_articles.view_count`, so the answer is on screen before anyone goes looking. Clicking one opens the **existing** reader (`help-centre.php?id=`) — there is deliberately no second article viewer. Fetched **outside** the awaited dashboard load, so a slow knowledge base cannot delay the tickets the page exists for. `ORDER BY` cannot take a bound parameter, so the sort is a fixed two-option allow-list matched to a literal, never interpolated from the request.
- 🔑 **One definition of "what may this requester read"** (`includes/knowledge/portal_reader.php`): `portalKnowledgeScope()` returns the `WHERE` fragment and params for *published AND not archived AND audience ≥ customer AND (their company OR shared)*, and every portal surface that lists or opens an article goes through it — the Knowledge list, the single article, ticket deflection and the dashboard. It had been copy-pasted across two endpoints; the dashboard would have been a third. **Duplicated scope is the dangerous kind of duplication**: nothing breaks when the copies drift, one of them just quietly starts showing more than it should, and there is no failing test for that. The function also makes both knowledge traps unreachable rather than merely documented — it uses `knowledgeTenantFilterForCompany()` (NULL = shared with every company, the inverse of tickets) and checks `is_archived` as well as `is_published` — and hard-codes `Audience::CUSTOMER` rather than accepting it, so a caller cannot declare how trusted it is. `portalArticlePreview()` shares the preview cleaner, which drops `<script>`/`<style>` **whole** (`strip_tags` keeps tag *contents*) and inserts a separator before each tag so a heading doesn't run into the sentence after it.
- **Compose screen** (`new-ticket.php`): laid out like writing an email rather than filling in a form — subject across the top, a **full-height TinyMCE editor** filling the page (bold, italics, lists, links; a deliberately short toolbar), and a right-hand settings panel for everything you merely *set*: mailbox, priority, attachments, screen recording. Full height is a **flex chain with `min-height: 0` on every ancestor**, not a `calc(100vh - …)` magic number (which was the first attempt and wrong at every window size but one). Editor HTML is passed through `sanitiseUserHtml()` (`includes/html_sanitise.php`) server-side; `<img>` is deliberately **not** on the allow-list, since a remote image in a customer-authored body would beacon the reading analyst's IP address and read time.
- **My Tickets, two-pane** (`tickets.php`): the list stays on screen on the left, the conversation loads on the right, and the reply box is docked at the bottom — much closer to an email client than the old page-per-ticket. The status filter is a **dropdown built from the statuses actually present in that person's tickets**, replacing a fixed Open/Closed/All toggle. `ticket.php` remains **only as a redirect**, because `?id=` links are already bookmarked and shared. Two traps live here: selection precedence is `ssSelected || window.PAGE.ticketId || first` and is **awaited** (an un-awaited auto-select used to race `sendReply()` and throw you back to your first ticket), and the classes are prefixed `tk-` **not `ss-`, because `user-menu.php` emits its `<style>` after the page's and owns that prefix** — an `.ss-msg { display: none }` from the menu once rendered the whole thread blank.
- **Screen recording is a modal** (`.rec-modal`): it was an inline panel that shoved the rest of the form around and left the video preview no room. It has its own Cancel, and the *Use this* submit guard reopens it rather than a panel.
- **Recording when REPLYING, from one shared component** (`assets/js/screen-recorder.js` + `self-service/includes/record-modal.php`): the reply composer on `tickets.php` offers the same **Record screen** button as the compose screen. Recording was previously only available on the way in, so the moment an analyst asked "can you show me what happens?" the one thing that would have answered it wasn't there. Capture, MIME negotiation, the countdown, the preview and the claim-or-discard decision are **identical** whichever screen you are on, so they live in one module rather than two copies that would drift — the seam between module and page is a single `onClaimed` callback saying what a claimed recording attaches to. The `<script>` is loaded per-page via `$needsRecorder` rather than globally (it is dead weight on the dashboard) and must be present before the page block, which calls `ScreenRecorder.init()` at parse time. `ScreenRecorder.isSupported()` hides the button on iOS Safari, which has no `getDisplayMedia` at all, and `hasUnclaimed()` drives the same submit guard on both screens.
- 🔑 **Claiming a pending recording is also one function** (`includes/ticket_recordings.php`, `claimPendingRecordings()`): recordings must be uploaded *before* the thing they belong to exists (people re-record until happy), so they land pending and are claimed when the ticket or reply is written. **The ownership guard is the part that must never exist in two versions** — a row is claimable only while `ticket_id IS NULL` **and** `recorded_by_user_id` is the same user, applied in the query so someone else's id simply selects nothing. Recording ids are sequential and guessable, so that guard is the only thing standing between them. Claiming runs *after* the commit in both callers: file moves are not transactional, and a recording that fails to move must never cost someone the reply they just wrote.
- **`ticket_recordings.email_id`** (NULL = the ticket's opening message): a recording now sits against the message it came with, so a video attached to reply #7 renders there instead of at the top of the thread. Every pre-existing row is correct as NULL, so there is **no backfill**. The FK is `ON DELETE SET NULL`, not CASCADE — if a message is ever removed the video should fall back to the ticket, not be destroyed with it.
- **Recordings play in the portal again**: the two-pane rebuild dropped the player entirely, so `get_ticket_detail.php` kept returning recordings that nothing rendered — a customer who recorded their screen when raising the ticket could no longer watch it back. Playback is inline `<video>` against the range-aware `get_recording.php`, so seeking doesn't pull the whole file.
- **Portal confirmations use the app-wide toast** (`assets/js/toast.js`): the same notification the analyst side uses, self-contained and falling back to sensible defaults for the position/animation settings that are analyst preferences the portal has no equivalent of — instead of each portal page inventing its own message strip.
- **Screen recordings**: One-click HTML5 screen capture via `getDisplayMedia()` + `MediaRecorder`, offered both when **raising** a ticket and when **replying** to one &mdash; no plugins, no third-party tools, no Loom install. Users pick whether to include microphone audio (off by default), record for up to 5 minutes with a live countdown, preview the result, then attach it to their ticket. Browser produces MP4/H.264 directly on modern Chrome/Edge 134+ and falls back to WebM/VP9 elsewhere &mdash; both play natively in the analyst's reading pane. Files land on disk under `recordings/{ticket_id}/` (never as DB blobs) and stream via a range-aware endpoint, so the analyst's `<video>` can seek without downloading the whole file. iOS Safari has no `getDisplayMedia` support, so the Record button is hidden on those browsers.
- **Ticket conversation** (`tickets.php`, right pane): the full thread and non-internal notes. Internal analyst notes are hidden from the portal. Any screen recordings the user attached appear as inline `<video controls>` cards above the conversation.
- **Replying** (`api/self-service/reply_ticket.php`): The portal used to be entirely **read-only** — a requester could watch their ticket but not answer it, so "can you send us a screenshot?" pushed them back to email, the exact round trip the portal exists to remove. A reply is stored as an `emails` row with `direction = 'Portal'` (the same shape `create_ticket.php` writes for the opening message, so it renders with the existing badge) and `is_initial = 0`, inheriting the thread's `mailbox_id` so the analyst's next outbound reply still goes out from the address the requester has been talking to. **Nothing is sent**: the requester is already here and the analyst sees it in the inbox, which also keeps the portal working on an install with no mailbox configured. Analysts' outbound replies quote the whole thread, so a portal reply is carried into their next email automatically. Ownership (`tickets.user_id`) is the only guard, and a miss returns the same "Ticket not found" as `get_ticket_detail.php`, so the endpoint can never confirm someone else's ticket exists; deleted tickets are excluded. Requester text is escaped on the way **in** (`nl2br(htmlspecialchars())`) because it is echoed into the analyst's reading pane and quoted into outbound email.
- **Attachments both ways**: replies take files (same base64 payload shape and `tickets/attachments/{floor(id/1000)}/{email_id}/` convention as inbound mail, so the analyst's existing `get_attachment.php` serves them unchanged), and `api/self-service/get_attachment.php` finally lets a requester **download** what an analyst sent them — previously impossible, so a file mailed to someone was unreachable from the one place they were told to look. It is the portal twin of the analyst endpoint, differing only in the authorisation model: the analyst version asks *"can this analyst reach the ticket's company"*, this one asks *"did this requester raise this ticket"*, both resolving the ticket **through** the attachment's email rather than trusting the URL, so ids can't be enumerated. Inline attachments (embedded signature logos, pasted screenshots) are excluded from the list so a "spacer.gif" doesn't appear under every message. Serves `nosniff` with images/audio/video/PDF inline and everything else as a download, plus a `realpath()` containment check before reading.
- **Message bodies are sanitised** (`safeMessageHtml()`): email bodies are arbitrary third-party HTML — anyone who can email the service desk controls them — and were previously injected into the page raw. They are now parsed in an inert `DOMParser` document with dangerous tags, **every inline event handler** and `javascript:`/`data:text/html` URLs stripped before rendering. Removing `<script>` alone is not enough: an `<img src=x onerror=…>` *does* fire when assigned via `innerHTML`.
...
- **Three AI helpers** (`api/lms/ai_author.php`, provider configured on **LMS → Settings**): turn a **knowledge-base article into a lesson** (grounded in an article you already know to be true — your documented fixes become your training), draft a **course outline** from a topic, and write **quiz questions** from a lesson you have written. **Article source is SHARED articles only** (`knowledge_articles.tenant_id IS NULL`) — enforced in `includes/lms/knowledge_source.php` and applied to *both* the picker and the body read, since the picker only posts an id and `ai_author.php` is gated on `Cap::LMS_MANAGE` rather than on Knowledge. Deliberately not "the author's own companies": nothing in the LMS carries a company (`lms_courses`/`lms_lessons`/`lms_course_assignments`/`lms_learning_groups`/`lms_progress`), so a lesson built from one client's article is shown to *every* learner regardless — scoping the picker would close the article leak and open a lesson leak. Shared-only is the only rule coherent with an install-wide course; revisit if the LMS ever gains its own company. Every helper produces a *draft in the editor* — the AI never writes to the database, and generated questions go through the same validation a hand-typed one does, so a malformed answer key is rejected before it can reach a learner. Unconfigured, the editor still works; you just author by hand.
...
- LDAP / Active Directory sign-in — check passwords against your existing directory, auto-create people on first login, and gate access by directory group (see *LDAP / Active Directory* below)
...
### Knowledge article visibility (company + audience)
- **Two orthogonal columns on `knowledge_articles`, answering different questions.** `tenant_id` = *whose article is it*; `audience` = *who may read it*. You need both: scoping an article to a company still hands it to that company's anonymous website visitors unless something says it's internal.
- **`tenant_id` — ⚠️ NULL means SHARED WITH EVERY COMPANY here**, the opposite of what it means for tickets/assets/changes (where NULL = "unassigned, treat as the Default company's", visible only while Default is active). An MSP's generic "how to reset your password" should serve every client, so a NULL article is visible to all. Consequently Knowledge has its **own** filter helper (`knowledgeTenantFilterForCompany()`) and must never use `activeTenantFilter()` — that would silently unshare every global article from every non-Default company. The FK is deliberately **not** `ON DELETE SET NULL` (unlike `fk_assets_tenant`) for the same reason: it would promote a deleted company's private articles into everyone's.
- **`audience` — an ordered ladder, not a flag per channel**: `internal` (analysts only) → `customer` (+ signed-in self-service users) → `public` (+ anonymous web chat visitors). Each rung includes the more open ones, and a reader declares its own rung and gets everything at or above it. **⚠️ `customer` has no reader yet** — the self-service portal has no knowledge section (it never queries `knowledge_articles`), so an article marked `customer` is today identical to `internal` in effect. The rung exists because the ladder describes the *reader's trust*, not which features happen to be built: when the portal grows a knowledge section it reads at this level and those articles light up with no migration. The editor says so plainly rather than implying it works. The readers aren't peers — they're degrees of trust, and a web chat visitor is *anonymous* (they typed a name and email; neither is verified). A flag per channel would allow eight combinations, most of them nonsense ("visible to web chat but not analysts"), and each new channel would mean another column plus a retroactive decision about every existing article. "Self-service yes, web chat no" is simply `customer`. Values are `Audience::` class constants for the same reason as `Cap::` — on a visibility check, a typo that silently matches nothing is a disclosure rather than a bug.
- **Free migration, safe default.** `tenant_id` NULL = shared = exactly the pre-multi-tenancy behaviour, so nothing needs backfilling; embeddings encode `title + body` only, so ownership isn't baked in and nothing needs regenerating. `audience` defaults to **`internal`**, so running Database Verify can never *start* disclosing existing articles — authors opt in per article. Both filters are no-ops at N=1 and on installs that haven't migrated yet.
- **Enforced in SQL before scoring** (`includes/knowledge/kb_ai.php`), which is both the security boundary and less work than decoding vectors that would be discarded. `kbRetrieveArticles()` defaults to the *most restrictive* rung, so a future caller that forgets to think about scope gets too little rather than too much.

### LDAP / Active Directory
- **Directory sign-in, not SSO** — people type their directory password into FreeITSM's own login form and it's checked against the directory by **bind**. Configured alongside OIDC on the same **System → Authentication** page, as a second `protocol` on the shared `auth_providers` table (`protocol='ldap'`), so it reuses the provider UI, JIT flag, default modules, tenant scoping, identity links and session handling wholesale. Requested in GitHub #47.
- **Two-step bind** — a user types `alice`, not a DN, so every sign-in: binds as a read-only **service account** → **searches** for the person to find their DN → **re-binds as that DN** with the typed password (a successful bind *is* the password check; no hash is ever read) → re-binds as the service account to read their **groups**. Works against Active Directory, OpenLDAP, FreeIPA and 389 — every filter and attribute name is configurable, with one-click **Active Directory** / **OpenLDAP** presets, because a hard-coded flavour silently breaks the other.
- **Access by group (the gate)** — naming an **analyst group** and/or a **self-service user group** decides who may sign in: analyst-group members get an analyst account, user-group members a self-service account, and **anyone in neither cannot sign in at all, even with a correct password**. Both blank = no gate (anyone the directory knows becomes an analyst) — the single-team default. The gate is what makes JIT safe: without it, auto-create turns every employee in the directory into an analyst. It **fails closed** (an unreadable group list denies, never grants) and runs *before* provisioning, so a denied user leaves no trace. **Nested groups** are resolved on AD via `LDAP_MATCHING_RULE_IN_CHAIN` — a plain `memberOf` read only ever shows *direct* groups.
- **Just-in-time provisioning** — an analyst-group member with no account gets one built from their directory entry (username deduplicated, name + email mapped, an unusable random local hash, per-provider default modules). Accounts are linked to the directory's **immutable id** (`objectGUID` on AD — binary, hex-encoded; `entryUUID` on OpenLDAP), never the DN, so a rename or OU move can't orphan or duplicate them. **A directory entry with no email address is provisioned anyway** — plenty of staff (warehouse, shop-floor) are never given a mailbox, and the directory bind has already proven who they are, so an address adds nothing. Safe because the identity is the immutable id, not the email, and the match-by-email step is skipped for a blank address (otherwise a second mailbox-less starter would match the first).
- **Portal directory sign-in, including staff with NO mailbox** (`api/self-service/login.php`, `ldapResolveUser()` / `ldapPortalProviders()` in `includes/ldap.php`): the self-service login now behaves exactly like the analyst one — type an email **or** a username, and it binds if the account is pinned to a directory, checks a local hash if it is not, and asks the directories (JIT) if the account does not exist. This completes the two-group design: until it landed, a `user`-group member was refused analyst access **and** could not use the portal, so half the gate was a promise nothing kept. 🔑 **`users` gained a `username` column and `users.email` became NULLable** — the analyst-side fix (#872) worked without any schema change only because `analysts.email` is not unique, so a blank could repeat; `users.email` **is** unique, so the second mailbox-less person collided with the first. An absent address is therefore stored as **NULL, never `''`**: MySQL permits many NULLs in a unique index but only one empty string. `emails.from_address` became NULLable for the same reason — it was NOT NULL, so a mailbox-less requester could sign in and then fail to raise a ticket at all.
- ⚠️ **Portal provider candidates are SCOPED, unlike the analyst side** (`ldapPortalProviders()`): the analyst login may try every enabled directory because analyst directories are global and there is one set of them. The portal cannot — on an MSP install each client company can own a directory, so trying them all would offer a password typed for one employer to every other client's domain controller, **and increment the AD lockout counter on each**, letting one person's typo lock accounts at a different company. Single-company installs try everything (no boundary exists); multi-company installs resolve the company from the email domain and try that company's directories plus global ones. **Known limit:** on a multi-company install a bare username can only match a global directory — nothing in `w.noemail` says which company they are in. Documented rather than guessed, because guessing here means spraying passwords.
- 🔑 **The company comes from the PROVIDER first, then the email domain** — `resolveTenantForNewUser()` derives the company from the address domain, which returns nothing for someone who has no address, i.e. exactly the people this feature exists for. A directory owned by a company vouches for its people; the domain is the fallback for everyone else.
- **Registration cannot claim a directory-backed account** (`api/self-service/register.php`, `self-service/verify-email.php`): these accounts have no local password by design, so they looked "unclaimed" to the existing check. Email confirmation meant no takeover was possible, but the real owner could innocently register and set a password that silently never works, since a pinned account routes to its directory and never checks a local hash. Both the registration and the token-redemption paths now refuse — the second independently, because a token minted before the account was linked must not plant a password on it afterwards.
- **Analysts see a name where there is no address**: one `senderLabel()` helper in `inbox.js` covers the list, reading pane, thread, search and attachments, so nobody renders `Wendy Warehouse <>`; three further sites built the label by string concatenation and would have printed the literal word `null`. Reply pre-fills from the requester's address if there is one, otherwise leaves the box empty and says a note is how you reach someone with no mailbox. `send_email.php` now **validates** the recipient — nothing on that path ever did, so `"null"` (which is not empty) would have been handed to Graph/Gmail/SMTP.
- **Empty-password guard** — RFC 4513 defines a bind with a DN and an empty password as an *unauthenticated bind*, which many directories answer with **success**; without an explicit check, a blank password box would log you in as anyone. Rejected before any network call (and again independently by the login form). Note both local test rigs happen to reject it, so no amount of testing proves the guard is present — it must never be removed.
- **Encryption + hardening** — `none` / STARTTLS / LDAPS per provider (the UI warns that passwords cross the network in the clear on `none`, and that AD commonly refuses password binds over plain LDAP). Referral-chasing is disabled (AD answers subtree searches with referrals that otherwise hang the search); connect/read timeouts keep a dead DC off the login page; the login string is `ldap_escape`d **before** filter substitution. The service account's password is encrypted at rest and never returned to the browser. Local password expiry never applies to directory-backed accounts (there's no local password to expire), but FreeITSM's own TOTP **still can** — unlike SSO, the directory doesn't own MFA.
- **Admin *Test* button** (`api/system/test_ldap_connection.php`) — proves connect → service bind → base DN readable, and optionally runs a full sign-in for one user, reporting the name/email/**groups** found and the **access it worked out**. It returns the real error (the login form never does), which is how you spot a wrong attribute, a too-narrow base DN, or a group gate that's silently denying everyone.
