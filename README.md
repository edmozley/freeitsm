...
> **Status: Stage 1 — engine foundation.** Schema (`workflows` + `workflow_executions`), engine class (`workflow/includes/engine.php`) with trigger/operator/action catalogues + `dispatch(event, payload)` + `manualFire(workflowId, payload)`, form-based editor, list landing page with active/inactive status pills, "Test fire" button for synthetic-payload runs, recent-executions sidebar. v1 action handler is `log_message` so the engine is exercisable end-to-end without host modules wired up yet. Visual canvas builder + AI co-author + real action handlers (set ticket status, send email, create task, Graph API actions) + dry-run + Watchtower integration + starter recipes all planned for subsequent commits.
