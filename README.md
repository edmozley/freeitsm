...
- **Roles** (`system/roles/`): Grant a non-administrator the right to manage **one specific module's settings** without making them a full System administrator — RBAC "Layer 2", the fine control that sits on top of module access. A role bundles *capabilities*, assigned to analysts and/or whole teams. Permissions go down to the individual **settings tab**, so you can let someone own the ticket statuses and SLAs while keeping them out of the mailbox credentials — with a per-module "manage everything here" umbrella for the common case, and a **sensitive** badge on any capability that reaches credentials, email or money. Deny by default; System administrators bypass the layer entirely. Design doc: `docs/design/rbac.md`.
  ...
  - **Converted so far:** LMS (`lms.manage`), Asset Management (7 capabilities + `assets.manage` umbrella), Contracts (7 capabilities + `contracts.manage` umbrella), Change Management (5 capabilities + `changes.manage` umbrella), Tasks (5 capabilities + `tasks.manage` umbrella).
