...
- **API** (`system/api/`): Manage keys for the public REST API (`api/v1/`) and explore it interactively
  ...
  - **Granular permissions** per key: a resource × action matrix (tickets read/create/update/delete/restore, notes, conversation, audit, SLA, time entries; assets read/create/update, assignments, history, inventory; problems read/create/update/delete, journal, audit, incident/change links; changes read/create/update/delete, comments, audit, CAB read/manage/vote; knowledge read/create/update/delete/restore/purge, versions; requesters, analysts, companies, reference data) — keys start from zero
