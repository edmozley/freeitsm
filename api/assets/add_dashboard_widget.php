...
try {
    ...
    // Get next sort order
    $maxStmt = $conn->prepare("SELECT COALESCE(MAX(sort_order), 0) + 1 AS next_order FROM analyst_dashboard_widgets WHERE analyst_id = ?");
    $maxStmt->execute([$analyst_id]);
    $nextOrder = (int)$maxStmt->fetch(PDO::FETCH_ASSOC)['next_order'];

    $stmt = $conn->prepare("INSERT IGNORE INTO analyst_dashboard_widgets (analyst_id, widget_id, sort_order) VALUES (?, ?, ?)");
