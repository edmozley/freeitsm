...
return [
    ...
    'help' => [
        ...
        'actions_row6_does' => 'Sends an email to the ticket\'s requester (or any address) using the ticket\'s mailbox. Subject is prefixed with <code>[SDREF:&hellip;]</code> so replies thread back automatically. <strong>A ticket is now optional</strong> &mdash; choose a mailbox to send from instead, which is how a form acknowledges a submission before any ticket exists. Without a ticket there is nothing for a reply to thread onto, so the &ldquo;reply above this line&rdquo; marker is left off rather than inviting a reply that would go nowhere.',
        ...
        'actions_row8_does' => 'Creates a new ticket. Useful for fan-out workflows like <em>"new-starter form &rarr; IT + HR + Facilities tickets"</em>. When the event carries a <strong>form submission</strong>, the ticket is raised the good way automatically &mdash; the requester matched to their real account, the company taken from it, and the submitted answers laid out as a table if you leave the body blank. Anything you set here still wins; the form only fills in what you left empty.',
        ...
        // Chaining (#95)
        'variables_chain_heading' => 'Using what an earlier action produced',
        'variables_chain_intro'   => 'Actions run in order, and a later one can refer to what an earlier one did. This is what makes <em>&ldquo;raise a ticket, then email the person their ticket number&rdquo;</em> a single workflow rather than an impossibility.',
        'variables_chain_last'    => '<code>{{last.ticket_id}}</code> &mdash; whatever the action immediately before this one produced.',
        'variables_chain_steps'   => '<code>{{steps.1.ticket_number}}</code> &mdash; a particular action, by the number shown beside it on the canvas. Use this when the action you want is not the one directly before.',
        'variables_chain_note'    => 'These deliberately sit under their own names rather than overwriting the workflow&rsquo;s own data. If creating a ticket replaced <code>{{ticket.id}}</code>, every later step on a ticket workflow would quietly start pointing at the new ticket instead of the one that set the workflow off.',

        // Forms (#95)
        'actions_forms_callout' => '<strong>Forms can run these actions too.</strong> Rather than building a workflow, open a form and use its <strong>What happens next</strong> tab to say what should happen when it is submitted, approved or rejected. It is the same engine and the same actions, and the runs appear in the same execution log &mdash; the form editor is simply a second way in, scoped to that one form. Build a workflow here when a rule should span several forms.',
        ...
        'triggers_family_forms'  => '<strong>Forms and catalogue requests</strong> &mdash; <code>form.submitted</code> fires for an ordinary form. A form that needs approval fires <code>catalogue_request.submitted</code> instead, then <code>catalogue_request.approved</code> or <code>catalogue_request.rejected</code> once your approver has decided. The separate event is deliberate: it means a rule that raises a ticket on submission cannot step over an approval you asked for. All four carry the submitted answers, so <code>{{submission.fields.Device type}}</code> works at every stage, and all four can be narrowed to a single form.',
