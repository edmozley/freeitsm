...
<div class="help-section" id="overview">
    <div class="help-section-header"><?php echo helpSectionNum('overview'); ?>
        <div>
            <h3>What Companies is for</h3>
            <p>A <strong>company</strong> is the user-facing word for a tenant — one of the separate client organisations you support. The Companies page is where you create them, and where you tell FreeITSM how to recognise each one's inbound email so it lands in the right place.</p>
        </div>
    </div>
...
<div class="help-section" id="adding">
    <div class="help-section-header"><?php echo helpSectionNum('adding'); ?>
        <div>
            <h3>Adding and editing companies</h3>
            <p>Each company is just a name and an active flag to begin with. You add the email routing once it exists.</p>
        </div>
    </div>
    <div class="help-steps">
        <div class="help-step"><div class="help-step-num">1</div><div><strong>Click Add.</strong> Give the company a name your team will recognise (e.g. the client's trading name). New companies are <strong>Active</strong> by default.</div></div>
        <div class="help-step"><div class="help-step-num">2</div><div><strong>Save.</strong> Adding the second company is what flips the install into multi-company mode — the email-routing fields appear from then on.</div></div>
        <div class="help-step"><div class="help-step-num">3</div><div><strong>Re-open the company to configure email.</strong> The email domains, specific senders and the routing summary only show when you <em>edit</em> an existing company on a multi-company install — there's nothing to route with a single company.</div></div>
    </div>
...
<div class="help-section" id="email">
    <div class="help-section-header"><?php echo helpSectionNum('email'); ?>
        <div>
            <h3>Routing inbound email by domain</h3>
            <p>When several companies share one inbox, FreeITSM needs to tell whose ticket is whose. An <strong>email domain</strong> is the main key: mail from anyone at that domain is filed under that company.</p>
        </div>
    </div>
    <ul>
    ...
    <div class="help-note warn"><strong>Use real, owned domains.</strong> Don't map a public/free-email domain (like <code>gmail.com</code>) to a company — it would scoop up unrelated people. Those are handled by the public email-domains list and by specific senders instead.</div>
</div>

<!-- 4. Personal & free email -->
<div class="help-section" id="senders">
    <div class="help-section-header"><?php echo helpSectionNum('senders'); ?>
        <div>
            <h3>People on personal or free email</h3>
            <p>Not everyone writes in from a company domain. A contractor or a small-business contact might use a personal Gmail or Outlook address that can't safely be mapped by domain. Two tools cover this.</p>
        </div>
    </div>
    ...
    <div class="help-note">Rule of thumb: <strong>domains</strong> route a whole organisation; <strong>specific senders</strong> route one individual; the <strong>public email-domains list</strong> protects shared providers from being mapped by accident.</div>
</div>

<!-- 5. Summary -->
<div class="help-section" id="summary">
    <div class="help-section-header"><?php echo helpSectionNum('summary'); ?>
        <div>
            <h3>How email reaches this company</h3>
            <p>Every company's edit screen has a read-only <strong>How email reaches this company</strong> panel. It's derived from everything above, so you can confirm the routing without guessing — change a domain or sender and the summary updates.</p>
        </div>
    </div>
    ...
    <div class="help-note ok"><strong>If the summary shows no route,</strong> the company won't receive any mail yet: add an email domain or a specific sender, or pin it an inbox, and the panel will fill in.</div>
</div>
