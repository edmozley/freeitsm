...
<div class="help-section" id="overview">
    <div class="help-section-header"><?php echo helpSectionNum('overview'); ?>
        <div>
            ...
            <p>FreeITSM holds a number of secrets it needs to connect to other systems — your vCenter password, AI API keys, and the Microsoft/Azure credentials used to read a mailbox. Rather than store these in plain text, it encrypts them with a single <strong>AES-256-GCM</strong> key.</p>
        </div>
    </div>
    ...
    <div class="help-note">The key lives in a single file on the server, outside the database. FreeITSM reads it whenever it needs to decrypt a stored secret, so the file must stay readable by the web server.</div>
</div>

<!-- 2. Key status -->
<div class="help-section" id="status">
    <div class="help-section-header"><?php echo helpSectionNum('status'); ?>
        <div>
            <h3>Reading the key status</h3>
        </div>
    </div>
...
<div class="help-section" id="setup">
    <div class="help-section-header"><?php echo helpSectionNum('setup'); ?>
        <div>
            <h3>Generating a key</h3>
            <p>If the status is <strong>missing</strong>, generating a key is a one-click, one-time step.</p>
        </div>
    </div>
    <div class="help-steps">
        <div class="help-step"><div class="help-step-num">1</div><div><strong>Open this page.</strong> If no key exists you'll see the red <em>missing</em> status with a <strong>Generate</strong> button.</div></div>
        <div class="help-step"><div class="help-step-num">2</div><div><strong>Click Generate.</strong> FreeITSM creates a fresh random AES-256 key and writes it to the key file on the server. The status refreshes to green when it succeeds.</div></div>
        <div class="help-step"><div class="help-step-num">3</div><div><strong>Back up the key file immediately</strong> (see below) before you start saving any credentials that depend on it.</div></div>
    </div>
    <div class="help-note warn"><strong>Regenerating replaces the existing key.</strong> If a key is already in use, generating a new one means anything encrypted with the old key can no longer be decrypted. Only regenerate when the current key is genuinely lost or invalid, and be ready to re-enter the affected credentials afterwards.</div>
</div>
...
<div class="help-section" id="backup">
    <div class="help-section-header"><?php echo helpSectionNum('backup'); ?>
        <div>
            <h3>Backups &amp; recovery</h3>
            <p>The key file is the one thing you must protect. Encrypted secrets are useless without it, and there is no way to recover them if it's gone.</p>
        </div>
    </div>
    <ul>
    ...
    <div class="help-note ok"><strong>Rule of thumb:</strong> generate the key once, back the file up immediately, store the backup safely, and don't regenerate unless you've truly lost it.</div>
</div>
