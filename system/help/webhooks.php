...
<div class="help-section" id="building">
    <div class="help-section-header"><?php echo helpSectionNum('building'); ?>
        <div>
            <h3>Building a webhook</h3>
            <p>Webhooks are built in the <strong>Workflows</strong> module. In short:</p>
        </div>
    </div>
    <div class="help-steps">
        <div class="help-step"><span class="help-step-num">1</span><div>Create a workflow and pick its <strong>trigger</strong> — the event you want to push out (ticket created, change approved, SLA breached, and well over a hundred more). The trigger <em>is</em> the webhook's event; there's no separate event list to tick.</div></div>
        <div class="help-step"><span class="help-step-num">2</span><div>Add conditions if you only want some of them — say, priority is P1.</div></div>
        <div class="help-step"><span class="help-step-num">3</span><div>Add the <strong>Send a webhook</strong> action. Choose a <strong>format</strong> (Slack, Microsoft Teams, Discord, the full record as JSON, or your own raw JSON), paste the <strong>URL</strong>, write the <strong>message</strong>, and optionally set a <strong>signing secret</strong>.</div></div>
        <div class="help-step"><span class="help-step-num">4</span><div>Press <strong>Send test</strong> in the action panel. It renders your message against a real recent record and posts it for real, straight away, showing you the exact headers and body sent and whatever came back.</div></div>
        <div class="help-step"><span class="help-step-num">5</span><div>Turn the workflow <strong>Active</strong>. That's the webhook's on/off switch.</div></div>
    </div>
    ...
    <div class="help-note"><strong>A test that passes does not prove live delivery works.</strong> The test is sent by the web server; real deliveries are sent by the background worker, which is a different PHP process with its own configuration. See Troubleshooting at the foot of this page.</div>
</div>

<!-- 3. The delivery worker -->
<div class="help-section" id="worker">
    <div class="help-section-header"><?php echo helpSectionNum('worker'); ?>
        <div>
            <h3>The delivery worker — do this first</h3>
            <p>FreeITSM never sends a webhook while you wait. The workflow drops it into a queue and returns instantly; a background worker picks it up and sends it. That keeps a slow or dead endpoint from hanging up the app — but it has one consequence you cannot ignore:</p>
        </div>
    </div>
    <div class="help-note warn"><strong>If the worker isn't scheduled, nothing is ever sent.</strong> Webhooks queue up silently and for ever. No error appears anywhere else in the app. This is the single most common reason for &ldquo;my webhooks don't work&rdquo;.</div>
    ...
    <div class="help-note"><strong>Time-based triggers need a second cron.</strong> Workflows fired by the passage of time rather than by an action — <code>sla.warning</code>, <code>sla.breached</code>, <code>contract.expiring</code>, <code>asset.warranty_expiring</code> — depend on the scheduled-workflow cron as well. Without it those triggers never fire at all, so their webhooks never even reach the queue.</div>
</div>

<!-- 4. The delivery log -->
<div class="help-section" id="log">
    <div class="help-section-header"><?php echo helpSectionNum('log'); ?>
        <div>
            <h3>The delivery log</h3>
            <p>One row per delivery attempt-chain: when it was queued, which workflow produced it, the format, the (redacted) URL, its status, how many attempts it has used, the last HTTP code it got and when it will next be retried. Filter with the chips along the top, and click a row to open it.</p>
        </div>
    </div>
...
<!-- 5. Replay -->
<div class="help-section" id="replay">
    <div class="help-section-header"><?php echo helpSectionNum('replay'); ?>
        <div>
            <h3>Replaying a delivery</h3>
            <p><strong>Replay</strong> puts a delivery back at the front of the queue, exactly as it was — same body, same signature — so the worker sends it again on its next run. It's how you recover the alerts that were lost while an endpoint was down, once you've fixed it.</p>
        </div>
    </div>
    ...
    <div class="help-note warn"><strong>Replay needs the payload, and the payload has an expiry.</strong> Once a delivery's stored body has been scrubbed by the retention setting below (7 days by default), it can never be replayed — there is nothing left to send. The row stays in the log for the record, but the Replay button explains why it can't run.</div>
</div>

<!-- 6. Payload retention -->
<div class="help-section" id="retention">
    <div class="help-section-header"><?php echo helpSectionNum('retention'); ?>
        <div>
            <h3>Payload retention</h3>
            <p>Webhook bodies can contain real ticket content — subjects, requester names, whatever your message includes. <strong>Keep sent payloads for</strong> decides how long that content sits in the database after the delivery has settled. Choose from: don't store them at all, 1 day, 7 days (the default), 30 days, or as long as the delivery record itself.</p>
        </div>
    </div>
    <p>Changing it applies immediately, and the page tells you how many stored payloads it scrubbed on the spot.</p>
    <div class="help-note"><strong>&ldquo;Don't store them at all&rdquo; doesn't mean the body is never written.</strong> Delivery is asynchronous, so the body has to survive in the queue until it's been sent — it simply gets wiped the instant the delivery settles. A queued webhook is never destroyed before it goes out.</div>
    <div class="help-note"><strong>Retention is a trade against Replay.</strong> Shorter retention is better for privacy; longer retention is what lets you re-send a week-old batch after an outage. &ldquo;Don't store them at all&rdquo; means you can never replay anything. Seven days is a reasonable middle.</div>
...
<div class="help-section" id="security">
    <div class="help-section-header"><?php echo helpSectionNum('security'); ?>
        <div>
    ...
    <div class="help-note warn"><strong>No encryption key means plain text.</strong> If no key is configured, FreeITSM stores these values unencrypted rather than refusing to work — and the red banner on the Data protection card says so. Set a key up on the Encryption page. Note that doing so does <em>not</em> retro-encrypt what's already saved: you must open and re-save each workflow that sends a webhook.</div>
...
<div class="help-section" id="trouble">
    ...
    <div class="help-note ok"><strong>The 30-second health check:</strong> status pill says <em>running</em>, success rate is near 100%, and <em>Queued now</em> is a small number that keeps falling. If queued only ever grows, the worker isn't running.</div>
</div>
